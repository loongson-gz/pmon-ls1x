...
static void codec_init(void)
{
	...
	i2c_master_send(&uda1342, buf, 3);
#elif defined(CONFIG_ES8388)
	struct i2c_client es8388;
	u8 buf[2];
	...
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x2b; buf[1] = 0x80;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x00; buf[1] = 0x32;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x01; buf[1] = 0x72;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x03; buf[1] = 0x00;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x04; buf[1] = 0x3c;
	i2c_master_send(&es8388, buf, 2);
	
	buf[0] = 0x09; buf[1] = 0x00;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x0a; buf[1] = 0x00;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x0c; buf[1] = 0x0c;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x0d; buf[1] = 0x02;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x0f; buf[1] = 0x70;
	i2c_master_send(&es8388, buf, 2);
	...
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x17; buf[1] = 0x18;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x18; buf[1] = 0x02;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x19; buf[1] = 0x72;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x1a; buf[1] = 0x00;
	i2c_master_send(&es8388, buf, 2);
	...
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x27; buf[1] = 0xb8;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x28; buf[1] = 0x38;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x29; buf[1] = 0x38;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x2a; buf[1] = 0xd0;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x2e; buf[1] = 0x21;
	i2c_master_send(&es8388, buf, 2);
	
	buf[0] = 0x2f; buf[1] = 0x21;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x30; buf[1] = 0x21;
	i2c_master_send(&es8388, buf, 2);
	buf[0] = 0x31; buf[1] = 0x21;
	i2c_master_send(&es8388, buf, 2);
